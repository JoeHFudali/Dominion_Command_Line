...
void Functionality::AgentPlayCard(Card c, vector<Player>& players, vector<Card>* hand, Deck* draw, Deck* discard, Board& b, int& actionCount, int& buyCount, int& coinCount, int& mBuff) {
	for (int i = 0; i < c.getDesc().size(); i++) {
		if (c.getDesc()[i].length() > 12) {
			decideAIAction(c.getName(), players, hand, draw, discard, b, actionCount, buyCount, coinCount, mBuff);
		}
		else if (c.getDesc()[i].find("+") != string::npos && c.getDesc()[i].find(" Card") != string::npos) {
			int amount = c.getDesc()[i][1] - '0';
			addCards(hand, draw, discard, amount);
		}
