...
void Game::setUpBoard(int numPlayers) {
	...
	do {
		...
		for (int i = 0; i < avaliableKingdomCards.size(); i++) {
			if (selected[i]) {
				cout << i + 1 << ". " << avaliableKingdomCards[i].getName() << " - [yes]" << endl;
			}
			else {
		...
		for (int i = 0; i < avaliableKingdomCards.size(); i++) {
			if (cName == avaliableKingdomCards[i].getName() && !selected[i]) {
		...
		if (!found) {
			cout << "Error, card not found or you choose an already choosen card. Try typing the name of an unselected card found above!" << endl << endl;
		}
...
void Game::setUpBoardRandomly(int numPlayers) {
	...
	while (numSelected < 10) {
		...
		int randCardIndex = rand() % avaliableKingdomCards.size();

		if (!selected[randCardIndex]) {
			numSelected++;

			kingdomCards.push_back(avaliableKingdomCards[randCardIndex]);

			selected[randCardIndex] = !selected[randCardIndex];
